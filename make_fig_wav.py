...
path='C:\\Program Files\\CDF_Distribution\\cdf38_1-dist\\lib'
os.environ["CDF_LIB"] = path
...
def get_wav_name(split_number,start,end,save_path,name):
    num_iter = str(split_number).zfill(2)
    s_num = str(start).zfill(8)
    e_num = str(end).zfill(8)
    return save_path+'\\'+name+'_split'+num_iter+'_'+s_num+'_'+e_num+'.wav'
...
#-------------------------------------------------
save_fig_dict = '.\\figure\\'+str(year)+'\\'+str(month).zfill(2)+'\\'
save_wav_dict = '.\\wavefile\\'+str(year)+'\\'+str(month).zfill(2)+'\\'
save_wform_dict = '.\\waveformfig\\'+str(year)+'\\'+str(month).zfill(2)+'\\'
wfc_dict = '.\\wfc\\'+str(year)+'\\'+str(month).zfill(2)+'\\'
mgf_dict = '.\\mgf\\'+str(year)+'\\'+str(month).zfill(2)+'\\'
wfc_list = glob.glob(wfc_dict+'*b_waveform_65khz*')
mgf_list = glob.glob(mgf_dict+'*v03.04*')
os.makedirs(save_fig_dict,exist_ok=True) #make directories to save
os.makedirs(save_wav_dict,exist_ok=True) 
os.makedirs(save_wform_dict,exist_ok=True) 
    ...
    for split_tmp in number_segment:
        ...
        for i in range(start,end,TIME_SCALE*Fs):
            ...
            if i+TIME_SCALE*Fs > end:
                ...
                new_end = end
            fig_name = get_fig_name(split_tmp, new_start, new_end, save_fig_dict, save_name)
            ...
            saveFigure(fig_name)
            #make a wavefile
            wav_name = get_wav_name(split_tmp, new_start, new_end, save_wav_dict, save_name)
            wf = wave.open(wav_name, 'w')
            wav_ch = 1
            wav_width = 2
            wf.setnchannels(wav_ch)
            wf.setsampwidth(wav_width)
            wf.setframerate(Fs)
            numsamples = TIME_SCALE*Fs
            x=np.linspace(start, end, numsamples) #numsamples等分
            y=B[new_start:new_end]
            y=np.rint(32767*y/5000)
            y=y.astype(np.int16)                 # 16ビット整数に型変換する
            y=y[0:numsamples]                    # numsamples 個のデータに打ち切る
            # ndarray から bytes オブジェクトに変換
            wav_data=struct.pack("h" * numsamples , *y)
            # データを書き出す
            wf.writeframes(wav_data)
            wf.close()
            #plot an waveform
            wform_name = get_fig_name(split_tmp, new_start, new_end, save_wform_dict, save_name)
            if os.path.exists(wform_name) == True: 
                continue
            initFigure()
            plt.plot(x,y,color=(0,1,1))
            time_array, fce, flag = time_setting(new_start,new_end)
            saveFigure(wform_name)
    print(wfc_name)
print(j)
print(k)
